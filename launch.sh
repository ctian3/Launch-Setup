...
mapfile -t instanceARR < <(aws ec2 run-instances --image-id ami-d05e75b8 --count 3 --instance-type t2.micro --key-name itmo544-ctian --security-group-ids sg-6aa5a90d --subnet-id subnet-20d5860b --associate-public-ip-address --iam-instance-profile Name=phpdeveloperRole --user-data file://../Environment-Setup/install-env.sh --output table |grep InstanceID | sed "s/|//g" |sed "s/ //g" | sed "s/InstanceId//g")
...
aws rds wait db-instance-available --db-instance-identifier ctian-db

aws rds create-db-instance-read-replica --db-instance-identifier ctian-db --source-db-instance-identifier ctian-db --db-instance-class db.t1.micro 
